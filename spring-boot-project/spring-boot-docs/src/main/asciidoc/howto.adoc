...
Furthermore, Spring Boot sanitizes the sensitive portion of URIs for keys which end with "uri", "uris", "address", or "addresses".
The sensitive portion of the URI is identified using the format `<scheme>://<username>:<password>@<host>:<port>/`.
For example, for the property `uri=http://user1:password1@localhost:8081`, the resulting sanitized value is
